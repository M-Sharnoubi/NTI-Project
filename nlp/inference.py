"""
Regex entity extraction and reusable single-message
inference function (usable from FastAPI, router, Streamlit, etc.).
"""
...
ORDER_ID_PATTERN = re.compile(
    ...
    |
    (?:اوردر|أوردر|طلب)\s*(?:رقم\s*)?\#?(?P<arabic>\d{3,})  # اوردر 1001
    """,
...
# ============================================================
# 23. CACHED MODEL LOADING
# ============================================================

@st.cache_resource(
    show_spinner="جاري تحميل نموذج MARBERT..."
)
def get_cached_model():
    """
    Load the trained MARBERT model only once.

    Streamlit caches this resource so the model, tokenizer,
    and configuration are reused across reruns and chat messages.
    """
    ...
    # Put model in evaluation mode once.
    model.eval()
...
def analyze_customer_message(
    ...
    """
    Analyze one customer message using the trained MARBERT model.

    If model/tokenizer/config are not provided, the cached model
    is loaded and reused instead of loading MARBERT again.
    """

    # --------------------------------------------------------
    # Load cached model only when caller does not provide one.
    # --------------------------------------------------------

    if model is None or tokenizer is None or config is None:
        model, tokenizer, config = get_cached_model()
    ...
    # inference_mode is appropriate because this is inference only.
    ...
    result = {
        "intent": config["id2intent"][str(intent_id)],

        "sentiment": config["id2sentiment"][str(sentiment_id)],

        "priority": config["id2priority"][str(priority_id)],
